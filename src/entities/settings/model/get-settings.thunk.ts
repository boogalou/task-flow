...
export const getSettingsRequest = createAsyncAction({
  actionType: 'settings/get',
  method: settingsService.getSettings,
});

export const getSettingsPending: CaseReducer<SettingsState> = (state) => {
  state.settingsFetchStatus = 'loading';
  state.error = null;
};

export const getSettingsFulfilled: CaseReducer<SettingsState, PayloadAction<UserSettings>> = (
  state,
  action,
) => {
...
export const getSettingsRejected: CaseReducer<SettingsState> = (state, action) => {
